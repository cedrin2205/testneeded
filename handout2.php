...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM handout2 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: handout2.php");
            exit();
        } else {
...
<html lang="en">
  ...
  <body>
        ...
        <section>
            ...
            <div class="contain">
                <form action="handoutupload2.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "handoutdb.php";
                $limit = 5; 
                $sql = "SELECT * FROM handout2 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($handout2 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($handout2['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                ...
                                echo '<img src="uploads/'.$handout2['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $handout2['filename']; ?></p>
                            <a class="delete-btn" href="handout2.php?delete_image=<?=$handout2['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$handout2['image_url']?>" download="<?=$handout2['image_url']?>">
