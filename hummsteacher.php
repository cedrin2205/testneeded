...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM hummsteacher WHERE image_url = '$imageToDelete'";
...
<div class="panel">
    <h2><font color = "yellow">List of HUMMS11 Student:</font></h2>
    ...
    include "hummsdb.php";
    $limit = 5; 
    $sql = "SELECT * FROM hummsteacher ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($hummsteacher = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($hummsteacher['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$hummsteacher['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $hummsteacher['filename']; ?></p>
                <a class="delete-btn" href="hummsteacher.php?delete_image=<?=$hummsteacher['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$hummsteacher['image_url']?>" download="<?=$hummsteacher['image_url']?>">
