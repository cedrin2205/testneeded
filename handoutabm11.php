...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM handoutabm11 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: handoutabm11.php");
            exit();
        } else {
...
<html lang="en">
  ...
  <body>
        ...
        <section>
            ...
            <div class="contain">
                <form action="handoutabm11upload.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "handoutdb.php";
                $limit = 15; 
                $sql = "SELECT * FROM handoutabm11 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($handoutabm11 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($handoutabm11['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                ...
                                echo '<img src="uploads/'.$handoutabm11['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $handoutabm11['filename']; ?></p>
                            <a class="delete-btn" href="handoutabm11.php?delete_image=<?=$handoutabm11['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$handoutabm11['image_url']?>" download="<?=$handoutabm11['image_url']?>">
