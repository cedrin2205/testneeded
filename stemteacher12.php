...
<div class="panel">
    <h2><font color = "yellow">List of STEM 12 Student:</font></h2>
    ...
    <div class="container">
        <h1>STEM 12 Teacher</h1>
        <a href="logout.php" class="btn btn-warning">Logout</a>
    </div>
    <div class="contain">
        <form action="stemupload12.php" method="post" enctype="multipart/form-data">
    ...
    include "stemdb12.php";
    $limit = 5; 
    $sql = "SELECT * FROM stemteacher12 ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($stemteacher12 = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($stemteacher12['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$stemteacher12['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $stemteacher12['filename']; ?></p>
                <a class="delete-btn" href="stemteacher12.php?delete_image=<?=$stemteacher12['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$stemteacher12['image_url']?>" download="<?=$stemteacher12['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "stem12teacher.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
