...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM gasteacher12 WHERE image_url = '$imageToDelete'";
...
<div class="panel">
    <h2><font color = "yellow">List of GAS 12 Student:</font></h2>
    ...
    <div class="container">
        <h1>GAS 12 Teacher</h1>
        <a href="logout.php" class="btn btn-warning">Logout</a>
    </div>
    <div class="contain">
        <form action="gasupload12.php" method="post" enctype="multipart/form-data">
    ...
    include "gasdb12.php";
    $limit = 5; 
    $sql = "SELECT * FROM gasteacher12 ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($gasteacher12 = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($gasteacher12['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$gasteacher12['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $gasteacher12['filename']; ?></p>
                <a class="delete-btn" href="gasteacher12.php?delete_image=<?=$gasteacher12['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$gasteacher12['image_url']?>" download="<?=$gasteacher12['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "gasteacher12.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
