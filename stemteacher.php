...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM stemteacher WHERE image_url = '$imageToDelete'";
...
<div class="panel">
    <h2><font color = "yellow">List of STEM 11 Student:</font></h2>
    ...
    include "stemdb.php";
    $limit = 5; 
    $sql = "SELECT * FROM stemteacher ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($stemteacher = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($stemteacher['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$stemteacher['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $stemteacher['filename']; ?></p>
                <a class="delete-btn" href="stemteacher.php?delete_image=<?=$stemteacher['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$stemteacher['image_url']?>" download="<?=$stemteacher['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "stemteacher.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
