...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM ictteacher WHERE image_url = '$imageToDelete'";
...
<div class="panel">
    <h2><font color = "yellow">List of ICT Students:</font></h2>
    ...
    include "ictdb.php";
    $limit = 5; 
    $sql = "SELECT * FROM ictteacher ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($ictteacher = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($ictteacher['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$ictteacher['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $ictteacher['filename']; ?></p>
                <a class="delete-btn" href="ictteacher.php?delete_image=<?=$ictteacher['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$ictteacher['image_url']?>" download="<?=$ictteacher['image_url']?>">
