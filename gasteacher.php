...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM gasteacher WHERE image_url = '$imageToDelete'";
...
<div class="panel">
    <h2><font color = "yellow">List of ABM Students:</font></h2>
    ...
    include "gasdb.php";
    $limit = 5; 
    $sql = "SELECT * FROM gasteacher ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($gasteacher = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($gasteacher['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$gasteacher['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $gasteacher['filename']; ?></p>
                <a class="delete-btn" href="gasteacher.php?delete_image=<?=$gasteacher['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$gasteacher['image_url']?>" download="<?=$gasteacher['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "gasteacher.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
