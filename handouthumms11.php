...
$allowedRoles = array('HUMMSteacher');
...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM handouthumms11 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: handouthumms11.php");
            exit();
        } else {
...
<html lang="en">
  ...
  <body>
        ...
        <section>
            ...
            <div class="contain">
                <form action="handouthumms11upload.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "handoutdb.php";
                $limit = 15; 
                $sql = "SELECT * FROM handouthumms11 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($handouthumms11 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($handouthumms11['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                ...
                                echo '<img src="uploads/'.$handouthumms11['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $handouthumms11['filename']; ?></p>
                            <a class="delete-btn" href="handouthumms11.php?delete_image=<?=$handouthumms11['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$handouthumms11['image_url']?>" download="<?=$handouthumms11['image_url']?>">
