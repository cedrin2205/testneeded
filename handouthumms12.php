...
$allowedRoles = array('HUMMSteacher12');
...
if (isset($_GET['delete_image'])) {
    ...
    $deleteQuery = "DELETE FROM handouthumms12 WHERE image_url = '$imageToDelete'";
    if (mysqli_query($conn, $deleteQuery)) {
        if (unlink("uploads/$imageToDelete")) {
            header("Location: handouthumms12.php");
            exit();
        } else {
  ...
  <body>
        ...
        <header>
           <div class="profile-strand">
                ...
                <p class="strand">Teachers Upload HUMMS</p>
            </div>
        ...
        <section>
            ...
            <div class="contain">
                <form action="handouthumms12upload.php" method="post" enctype="multipart/form-data">
            ...
            <div class="gallery">
                ...
                include "handoutdb.php";
                $limit = 15; 
                $sql = "SELECT * FROM handouthumms12 ORDER BY id DESC LIMIT $limit";
                $res = mysqli_query($conn, $sql);

                if (mysqli_num_rows($res) > 0) {
                    while ($handouthumms12 = mysqli_fetch_assoc($res)) { ?>
                        <div class="alb">
                            <?php 
                            $fileExtension = pathinfo($handouthumms12['image_url'], PATHINFO_EXTENSION);
                            if ($fileExtension === 'pdf') {
                                ...
                                echo '<img src="uploads/'.$handouthumms12['image_url'].'" alt="Uploaded Image">';
                            }
                            ?>
                            <p><?php echo $handouthumms12['filename']; ?></p>
                            <a class="delete-btn" href="handouthumms12.php?delete_image=<?=$handouthumms12['image_url']?>">
                                <img src="img/remove.png" alt="Delete" width="1" height="1">
                            </a>
                            <a class="download-btn" href="uploads/<?=$handouthumms12['image_url']?>" download="<?=$handouthumms12['image_url']?>">
