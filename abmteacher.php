...
<div class="panel">
    ...
    include "abmdb.php";
    $limit = 5; 
    $sql = "SELECT * FROM abmteacher ORDER BY id DESC LIMIT $limit";
    $res = mysqli_query($conn, $sql);

    if (mysqli_num_rows($res) > 0) {
        while ($abmteacher = mysqli_fetch_assoc($res)) { ?>
            <div class="alb">
                <?php 
                $fileExtension = pathinfo($abmteacher['image_url'], PATHINFO_EXTENSION);
                if ($fileExtension === 'pdf') {
                    echo '<img src="img/pdficon.png" alt="PDF File">';
                } elseif ($fileExtension === 'doc' || $fileExtension === 'docx') {
                    echo '<img src="img/wordicon.png" alt="Word File">';
                } else {
                    echo '<img src="uploads/'.$abmteacher['image_url'].'" alt="Uploaded Image">';
                }
                ?>
                <p><?php echo $abmteacher['filename']; ?></p>
                <a class="delete-btn" href="abmteacher.php?delete_image=<?=$abmteacher['image_url']?>">
                    <img src="img/remove.png" alt="Delete" width="1" height="1">
                </a>
                <a class="download-btn" href="uploads/<?=$abmteacher['image_url']?>" download="<?=$abmteacher['image_url']?>">
...
<script>
                ...
                xhttp.open("GET", "abmteacher.php?mark_as_seen=" + encodeURIComponent(imageUrl), true);
                xhttp.send();
